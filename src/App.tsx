...
// Loader with programmer icon and animated dots
function Loader() {
  return (
...
export default function App() {
  ...
  // Loader: wait for images and fonts
  useEffect(() => {
    const images = Array.from(document.images);
    let loaded = images.length === 0 ? 0 : images.filter((img) => img.complete).length;
    ...
    const check = () => {
      ...
      if (loaded === images.length) setTimeout(() => setLoading(false), 350);
    };
  ...
  return (
    <>
      ...
      <div className={`main-bg${dark ? " dark" : ""}${loading ? " bg-dimmed" : ""}`}>
        ...
        <main className="main-content" aria-hidden={loading}>
          {/* Header */}
